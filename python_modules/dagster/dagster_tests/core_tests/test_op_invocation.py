...
def test_op_invocation_with_resources():
    ...
    with pytest.raises(
        DagsterInvalidInvocationError,
        match='"op_requires_resources" has required resources, but no context was provided.',
    ):
...
def test_op_invocation_with_config():
    ...
    with pytest.raises(
        DagsterInvalidInvocationError,
        match='"op_requires_config" has required config schema, but no context was provided.',
    ):
