...
class AssetReconciliationCursor(NamedTuple):
    ...
    def from_serialized(cls, cursor: str, asset_graph: AssetGraph) -> "AssetReconciliationCursor":
        loaded_cursor = json.loads(cursor)
        if isinstance(loaded_cursor, list):  # backcompat
            (
                latest_storage_id,
            ...
            latest_storage_id = loaded_cursor["latest_storage_id"]
            serialized_materialized_or_requested_root_asset_keys = loaded_cursor[
                "materialized_or_requested_root_asset_keys"
            ]
            serialized_materialized_or_requested_root_partitions_by_asset_key = loaded_cursor[
                "materialized_or_requested_root_partitions_by_asset_key"
            ]
            serialized_last_observe_request_timestamp_by_asset_key = loaded_cursor.get(
                "last_observe_request_timestamp_by_asset_key", {}
            )
...
def get_auto_observe_run_requests(
    ...
    for asset_key in asset_graph.source_asset_keys:
        ...
        auto_observe_frequency_minutes = asset_graph.get_auto_observe_frequency_minutes(asset_key)

        if auto_observe_frequency_minutes and (
            last_observe_request_timestamp is None
            or (
                last_observe_request_timestamp + auto_observe_frequency_minutes * 60
                < current_timestamp
            )
        ):
